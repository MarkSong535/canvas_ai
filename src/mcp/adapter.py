    ...
    async def adapt(
        ...
        class MCPAdaptTool(AsyncTool):
            def __init__(
                ...
                self.original_tool_name = original_tool_name  # 保存原始工具名称
            ...
            async def forward(self, *args, **kwargs) -> str:
                if len(args) > 0:
                    if len(args) == 1 and isinstance(args[0], dict) and not kwargs:
                        async with client:
                            mcp_output = await client.call_tool(self.original_tool_name, arguments=args[0])
                    else:
                        raise ValueError(
                    ...
                    async with client:
                        mcp_output = await client.call_tool(name=self.original_tool_name, arguments=kwargs)

                # 处理CallToolResult对象
                if hasattr(mcp_output, 'content') and mcp_output.content:
                    # 如果有content属性且不为空，使用content
                    if isinstance(mcp_output.content, list) and len(mcp_output.content) > 0:
                        content = mcp_output.content[0]
                        if hasattr(content, 'text'):
                            return json5.loads(content.text)
                        else:
                            return str(content)
                    else:
                        return str(mcp_output.content)
                elif hasattr(mcp_output, 'text'):
                    # 如果有text属性，直接使用
                    return json5.loads(mcp_output.text)
                else:
                    # 否则转换为字符串
        ...
        tool = MCPAdaptTool(
            ...
            original_tool_name=tool.name,  # 传递原始工具名称
        )
